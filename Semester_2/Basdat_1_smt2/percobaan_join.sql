...
-- =====================
SELECT kode_jadwal, kode_kelas,
kode_mk, kode_ruang
FROM jadwal
WHERE jp_mulai = '002'
AND jp_selesai = '005'
ORDER BY kode_jadwal DESC;

SELECT COUNT (nama_dosen)'jumlah
dosen' from dosen;

SELECT COUNT(nama_dosen)
FROM dosen
WHERE nama_dosen LIKE '%DR.%' OR
nama_dosen LIke '%Ph.D%';

SELECT (kode_dosen) 'Kode
dosen',(nama_dosen) 'Nama Dosen'
FROM dosen
WHERE nama_dosen LIKE '%DR.%' OR
nama_dosen LIke '%Ph.D%'
...
WHERE p.kode_prodi=k.kode_prodi
ORDER BY nama_prodi ASC;

SELECT k.kode_kelas, k.nama_kelas,
p.kode_prodi, p.nama_prodi,
j.kode_jadwal, j.kode_ruang,
j.jp_mulai, j.jp_selesai
FROM kelas k
INNER JOIN prodi p ON k.kode_prodi
= p.kode_prodi
INNER JOIN jadwal j ON k.kode_kelas
= j.kode_kelas
...
j.kode_jadwal, j.kode_ruang,
j.kode_dosen,
d.nama_dosen, j.kode_hari,j.jp_mul
ai, j.jp_selesai
FROM kelas k
INNER JOIN prodi p ON k.kode_prodi
= p.kode_prodi
INNER JOIN jadwal j ON k.kode_kelas
= j.kode_kelas
INNER join dosen d ON j.kode_dosen
= d.kode_dosen
INNER join hari h ON j.kode_hari =
h.kode_hari
WHERE d.nama_dosen = 'ade ismail'
SELECT k.kode_kelas, k.nama_kelas,
p.kode_prodi, p.nama_prodi,
j.kode_jadwal, j.kode_ruang,
j.kode_dosen,
...
INNER join hari h ON j.kode_hari =
h.kode_hari
WHERE h.kode_hari = '001'
ORDER BY d.nama_dosen ASC;

SELECT p.nama_prodi,
COUNT(DISTINCT d.nama_dosen) AS
jumlah_dosen
FROM kelas k
INNER JOIN prodi p ON
k.kode_prodi = p.kode_prodi
INNER JOIN jadwal j ON
k.kode_kelas = j.kode_kelas
INNER JOIN dosen d ON
j.kode_dosen = d.kode_dosen
INNER JOIN hari h ON
j.kode_hari = h.kode_hari
